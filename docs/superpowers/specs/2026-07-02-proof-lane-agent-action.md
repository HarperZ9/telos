...
- Lookup order: `TELOS_EMET_CLI` environment variable (path to a runnable `emet.js` or executable), then a sibling checkout at `<telosRoot>/../emet/impl/js/emet.js`. No machine-specific absolute path is baked in: with no env override and no sibling checkout, resolution returns null and the witness records `unavailable` honestly. A developer who keeps an Emet implementation elsewhere (for example the pubscan checkout) points `TELOS_EMET_CLI` at it.
...
Target: `agent-action-proof-packet/v0` at proof-surface main `f8380da`. The 15 root field names are frozen in `export_contract.proof_surface_root_fields` and asserted by test. proof-surface is never imported or spawned at runtime.
...
| `context` | a plain object summary `{ workspace, tool_authority, context_refs? }` | Correction (verified against proof-surface HEAD `f8380da`, `agent_action/packet.py` `_validate_context`): proof-surface `context` is a plain object (a workspace and tool-authority summary), not `context_refs[]`, and it has no `envelope_hash` field. The Telos packet keeps its richer internal `context_refs[]` join, but `toProofSurfacePacket()` emits a plain `context` object; the internal refs are surfaced through `evidence_refs` instead. |
...
| `evidence_refs` | object `{ trace_refs, runtime_refs }` where each is `[{ ref, sha256? }]` | Correction (verified against proof-surface `agent_action/_evidence.py`): `evidence_refs` is an object with the buckets `trace_refs`, `eval_refs`, `model_refs`, `runtime_refs`, not a flat union list. The exporter surfaces the packet's context refs and route ref as `trace_refs` and its ledger ref as `runtime_refs`, each a `{ ref, sha256? }` object with bare 64-hex digests. |
| `failure_labels` | typed codes from the packet's failures, mapped into proof-surface's closed set | Correction (verified against proof-surface `_failure.py`): proof-surface `FAILURE_CODES` is a closed set `{binding_failed, unjoinable_action, verification_unverifiable, stale_criterion, authority_gap, evidence_gap, duplicate_idempotency_key, external_request_id_missing, failed_route}`. The Telos verifier codes are more specific, so the exporter maps each packet failure into the nearest proof-surface code (or emits `null`/`[]` when there are none, as the MATCH demo packet does). Absent or null is valid. |
...
| `decision_summary` | null, or derived from the packet's own overall `verifier.verdict` | Correction (verified against proof-surface HEAD `f8380da`, `_decision.py` `derive_decision_summary` and `DECISION_OUTCOMES`): `decision_summary` is null-or-derived-from-the-OVERALL-VERDICT, never from `admission.decision`. proof-surface derives it from the overall `MATCH`/`DRIFT`/`UNVERIFIABLE` verdict with the exact map `MATCH -> {decision: "approve", confidence: "high"}`, `DRIFT -> {decision: "block", confidence: "high"}`, `UNVERIFIABLE -> {decision: "escalate", confidence: "low"}`, each with `reason`, `next_action`, and `missing_evidence[]`. `DECISION_OUTCOMES` is `{approve, block, rerun, escalate, publish, deploy, archive}`; admission's `allow`/`deny`/`needs-human` are not valid decision values. `toProofSurfacePacket()` derives `decision_summary` from the packet's own overall verdict using that exact map, or emits `null`. |

Decision map (action-receipt `policy_decisions` -> proof-surface `ADMISSION_DECISIONS`, for the `admission[].decision` field only, not `decision_summary`): `allow` -> `allow`; `block` -> `deny`; `escalate` -> `needs-human`; `require_review` -> `needs-human`.
