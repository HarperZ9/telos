...
- Telos repo pre-checkpoint head `89ae91c`, branch `main`; latest in-turn Index refresh saw the active uncommitted performance-doctor edits.
...
- Telos catalog now presents 57 available tools across the five flagships, including `telos.context.pack`, `telos.model.foundry`, `telos.mcp.freshness`, `telos.ci.doctor`, `telos.presentation.doctor`, `telos.accessibility.doctor`, `telos.performance.doctor`, `telos.research.thermodynamic`, `telos.second_level.queue`, and `telos.workstation.substrate`.
