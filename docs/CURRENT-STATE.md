...
- Telos repo pre-checkpoint head `ab64060`, branch `main`; latest in-turn Index refresh saw the active uncommitted ci-triage edits.
...
The CI triage lane turns failed GitHub Actions logs into a native routing receipt. `node demo/ci-triage.mjs --summary` distinguishes blocking failures such as doctest/test-gate and Rust format failures from non-fatal Node runtime migration warnings, then emits `MATCH`, `DRIFT`, or `UNVERIFIABLE` as `telos.ci.triage`. Raw logs, tokens, secrets, private paths, workflow mutation, and GitHub writes are excluded; only hashes, redacted excerpts, typed failure codes, warning codes, and route hints leave the triage boundary.
