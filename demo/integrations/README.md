...
`browser-evidence.json` and `../browser-evidence.mjs` expose the browser evidence kernel for automated browsing and work actuation. Page state is represented by redacted refs, digests, side-effect classes, and verification verdicts so Index and Forum can route council/review work without duplicating browser stacks.
