...
`../mcp-freshness.mjs` emits `project-telos.mcp-freshness/v1`, a host-side stale-server probe contract. Hosts compare observed `serverInfo.version`, status `tool_version`, and `tools/list` hashes against the manifest before trusting a loaded MCP server. `../mcp-freshness.mjs --observed observed.json` emits `project-telos.mcp-freshness-observation/v1` with `MATCH`, `DRIFT`, or `UNVERIFIABLE` plus normalized failure codes.
