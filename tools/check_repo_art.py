...
The sibling Python repositories run these same gates under pytest. This one is
a Node repository, so the gates run as a script and emit a receipt that
demo/repo-art.test.mjs asserts on, the way demo/brand-renderer.test.mjs
already does for the flagship heroes.
"""
...
def check_seed_is_recorded(_unused: list[Path]) -> list[str]:
    ...
    svg = ART_LIB.header_svg(
        {"name": "telos", "role": "x", "tagline": "y", "words": []})
    stamp = f"SEED {ART_LIB.seed_for('telos') % 100000:05d}"
